...
void getAcc(const double pos[][3], const double mass[], double acc[][3],
            int N) {
    // TODO:
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j++) {
            if (i != j) {
                ...
                double inv_r3 = pow(
                    (pow(dx, 2) + pow(dy, 2) + pow(dz, 2) + pow(softening, 2)),
                    -1.5);
...
void savePositionsToCSV(const double pos[][3], int N, int step,
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        std::cerr
            << "Usage: " << argv[0]
            << " <number_of_particles> <simulation_end_time> <num_threads>"
            << std::endl;
        return 1;
    }
...
#pragma omp parallel for // schedule(static)
        for (int i = 0; i < N; i++) {
            pos[i][0] = std::min(pos[i][0], board_size);
            pos[i][1] = std::min(pos[i][1], board_size);
            pos[i][2] = std::min(pos[i][2], board_size);

            pos[i][0] = std::max(pos[i][0], -board_size);
            pos[i][1] = std::max(pos[i][1], -board_size);
            pos[i][2] = std::max(pos[i][2], -board_size);
        }
