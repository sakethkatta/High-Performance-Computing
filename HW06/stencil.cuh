...
// - The entire mask
// - The elements of image that are needed to compute the elements of output
// corresponding to the threads in the given block
// - The output image elements corresponding to the given block before it is
...
// You can consider following the kernel call with cudaDeviceSynchronize (but if
// you use cudaEventSynchronize to time it, that call serves the same purpose as
